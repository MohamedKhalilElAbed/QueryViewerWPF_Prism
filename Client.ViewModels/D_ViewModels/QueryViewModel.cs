...
namespace Client.D_ViewModels
{
    public class QueryViewModel : ViewModelBase
    {
        ...
        private ObservableCollection<string> _ColumnOriginalName;
        public ObservableCollection<string> ColumnOriginalName
        {
            get { return _ColumnOriginalName; }
            set { SetProperty(ref _ColumnOriginalName, value); }
        }
        private ObservableCollection<string> _ColumnUserDefinedName;
        public ObservableCollection<string> ColumnUserDefinedName
        {
            get { return _ColumnUserDefinedName; }
            set { SetProperty(ref _ColumnUserDefinedName, value); }
        }
        private ObservableCollection<int> _ColumnUserDefinedPosition;
        public ObservableCollection<int> ColumnUserDefinedPosition
        {
            get { return _ColumnUserDefinedPosition; }
            set { SetProperty(ref _ColumnUserDefinedPosition, value); }
        }
        ...
        public QueryViewModel(IRegionManager regionManager, IContainerProvider containerProvider, IEventAggregator ea)
        {
            ...
            _ea.GetEvent<ColumnRenameEvent>().Subscribe(ColumnRenameMessageReceived, ((object o,  string s1, string   s2, DataGrid dataGrid) arg) => arg.o == this);
            _ea.GetEvent<RefreshEvent>().Subscribe(Execute_Refesh, (object o) => o == this);
            _ea.GetEvent<AutoGeneratedColumnActionEvent>().Subscribe(AutoGeneratedColumn, ((object o, DataGrid datagrid) arg) => arg.o == this);

            ColumnOriginalName = new ObservableCollection<string>();
            ColumnUserDefinedName = new ObservableCollection<string>();
            ColumnUserDefinedPosition = new ObservableCollection<int>();
        }
        private void AutoGeneratedColumn((object, DataGrid) arg)
        {
            /*
            (object content, DataGrid dataGrid) = arg;
            for (int i = 0; i < dataGrid.Columns.Count; ++i)
            {
                dataGrid.Columns[i].DisplayIndex = ColumnUserDefinedPosition[i];
            }
            */
        }
        ...
        private void ColumnRenameMessageReceived((object, string , string , DataGrid) arg)
        {
            (object content,string label, string nouveauNom, DataGrid dataGrid) = arg;
            colHeader[label] = nouveauNom;
            /*
            if(ColumnOriginalName.Count == 0)
            {
                for(int i =  0; i < onp.Count; ++i)
                {
                    ColumnOriginalName.Add(onp[i]);
                    ColumnUserDefinedName.Add(onp[i]);
                    ColumnUserDefinedPosition.Add(i);
                }
            }
            ColumnOriginalName = new ObservableCollection<string>();
            */
            ColumnUserDefinedName[ColumnOriginalName.IndexOf(label)] = nouveauNom;
            dataGrid.Columns[ColumnOriginalName.IndexOf(label)].Header = nouveauNom;
        }
